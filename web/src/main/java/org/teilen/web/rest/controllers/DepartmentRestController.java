...
class DepartmentRestController {
    ...
    public List<Department> getDepartmentsByNameLike(@RequestParam("name") String name) throws Exception { // /api/department?name=departmentName
    ...
    public Department editDepartment(@PathVariable("id") String id, @RequestBody Department department)
            throws Exception {
        if (NumberUtils.isParsable(id)) {
            Long departmentId = Long.parseLong(id);
            if (department == null)
