...
class UserContactRestController {
    ...
    public List<UserContact> getUserContactByEmailLike(@RequestParam("email") String email) throws Exception {
    ...
    public List<UserContact> getUserContactByTelephone(@RequestParam("tel") String tel) throws Exception {
    ...
    public UserContact editUserContact(@PathVariable("id") String id, @RequestBody UserContact userContact)
            throws Exception {
        if (NumberUtils.isParsable(id)) {
            Long userContactId = Long.parseLong(id);
            if (userContact == null)
