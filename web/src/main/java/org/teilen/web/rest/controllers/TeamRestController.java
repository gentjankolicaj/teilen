...
class TeamRestController {
    ...
    public List<Team> getTeamsByDepartmentId(@RequestParam("departmentId") String departmentId) throws Exception {
        if (NumberUtils.isParsable(departmentId)) {
            Long depId = Long.parseLong(departmentId);
            return teamService.getByDepartmentId(depId);
        } else
            throw new IdException("Department id " + departmentId + " is not parsable.Must be an integer.");
    }
