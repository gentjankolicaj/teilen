...
class MessageRestController {
    ...
    public List<Message> getAllDeletedMessages(@RequestParam("deleted") String deleted) throws Exception {
    ...
    public List<Message> getAllUnDeletedMessages(@RequestParam("undeleted") String undeleted) throws Exception {
    ...
    public List<Message> getMessagesByTeamId(@RequestParam("teamId") String teamId) throws Exception { // /api/messages?teamId=12
        if (NumberUtils.isParsable(teamId)) {
            Long id = Long.parseLong(teamId);
            return messageService.getByTeamId(id);
        } else
            throw new IdException("Team id " + teamId + " is not valid and parsable.It must be an integer.");
    }

    @ResponseStatus(HttpStatus.OK)
    @ResponseBody
    @RequestMapping(params = {"senderId"}, method = RequestMethod.GET, produces = "application/json")
    public List<Message> getMessagesBySenderId(@RequestParam("senderId") String senderId) throws Exception { // /api/messages?senderId=12
        if (NumberUtils.isParsable(senderId)) {
            Long id = Long.parseLong(senderId);
            return messageService.getBySenderId(id);
        } else
            throw new IdException("Sender id " + senderId + " is not valid and parsable.It must be an integer.");
    }

    @ResponseStatus(HttpStatus.OK)
    @ResponseBody
    @RequestMapping(params = {"receiverId"}, method = RequestMethod.GET, produces = "application/json")
    public List<Message> getMessagesByReceiverId(@RequestParam("receiverId") String receiverId) throws Exception { // /api/messages?receiverId=12
        if (NumberUtils.isParsable(receiverId)) {
            Long id = Long.parseLong(receiverId);
            return messageService.getByReceiverId(id);
        } else
            throw new IdException("Receiver id " + receiverId + " is not valid and parsable.It must be an integer.");
    }
    ...
    public Message editMessage(@PathVariable("id") String id, @RequestBody Message message) throws Exception {
        if (NumberUtils.isParsable(id)) {
            Long messageId = Long.parseLong(id);
            if (message == null)
                throw new NullReferenceException("Message reference binded is null.");
    ...
    @ResponseBody
    @RequestMapping(params = {"teamId", "delete"}, method = RequestMethod.DELETE, produces = "application/json")
    public void deleteMessagesByTeamId(@RequestParam("teamId") String teamId, @RequestParam("delete") String delete)
    ...
    @ResponseStatus(HttpStatus.NO_CONTENT)
    @ResponseBody
    @RequestMapping(params = {"senderId", "delete"}, method = RequestMethod.DELETE, produces = "application/json")
    public void deleteMessagesBySenderId(@RequestParam("senderId") String senderId,
                                         @RequestParam("delete") String delete) throws Exception { // /api/messages?senderId=12&delete=true
        if (NumberUtils.isParsable(senderId)) {
            Long id = Long.parseLong(senderId);
            if (delete.equalsIgnoreCase("true"))
    ...
    @ResponseStatus(HttpStatus.NO_CONTENT)
    @ResponseBody
    @RequestMapping(params = {"receiverId", "delete"}, method = RequestMethod.DELETE, produces = "application/json")
    public void deleteMessagesByReceiverId(@RequestParam("receiverId") String receiverId,
                                           @RequestParam("delete") String delete) throws Exception { // /api/messages?receiverId=12&delete=true
        if (NumberUtils.isParsable(receiverId)) {
            Long id = Long.parseLong(receiverId);
