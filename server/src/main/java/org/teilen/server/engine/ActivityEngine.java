            ...
            if (packet instanceof ConnPacket) {
                ...
                if (roomPacket.roomOp.name().equals(RoomOp.ROOM_READ.name())) {
                    if (roomId != null) {
                        ...
                        if (room != null) {
                            RoomPacket rRoomPacket = new RoomPacket(new Header(-1, originId), new Body(null, new RoomWrapper(room)), roomId, RoomOp.ROOM_CREATE);
                                ...
                                if (clientIds != null && clientIds.size() != 0) {
                                    ...
                                    if (room != null) {
                                        ...
                                        for (Integer tmpClientId : clientIds) {
                                            RoomPacket tmpRoomPacket = new RoomPacket(new Header(-1, tmpClientId), new Body(null, new RoomWrapper(room)), roomId, RoomOp.ROOM_CREATE);
                    ...
                    if (body != null) {
                        ...
                        if (content instanceof RoomClientsWrapper) {
                            ...
                            if (clientIds != null && clientIds.size() != 0) {
                                ...
                                RoomPacket cRoomPacket = new RoomPacket(new Header(originId, originId), new Body(null, new RoomWrapper(room)), roomId, RoomOp.ROOM_CREATE);
                                //sent to requesting client
                                updateClientPacketMap(clientPacketMap, originId, cRoomPacket);

                                //sent to all clientIds
                                for (Integer tmpClientId : clientIds) {
                                    RoomPacket tmpRoomPacket = new RoomPacket(new Header(originId, tmpClientId), new Body(null, new RoomWrapper(room)), roomId, RoomOp.ROOM_CREATE);
                                    updateClientPacketMap(clientPacketMap, tmpClientId, tmpRoomPacket);
                                }
                            }
                        }
                            ...
                            if (content instanceof RoomClientsWrapper) {
                                    ...
                                    if (room.getOwnerId().intValue() == originId.intValue()) {
                                        ...
                                        for (Integer clientId : clientIds) {
                                            if (originId.intValue() != clientId.intValue()) {
                                                RoomPacket tmpRoomPacket = new RoomPacket(new Header(originId, clientId), new Body(null, new RoomClientsWrapper(clientIds)), roomId, RoomOp.ROOM_UPDATE);
                                        ...
                                        LogUtil.warn("RoomOp.UPDATE : Room with id " + roomId + " clients ids not updated.Room owner " + room.getOwnerId() + ", originId " + originId);
                                    }
                                } else
                                    LogUtil.error("RoomOp.UPDATE : Room with id " + roomId + " clients ids not updated.Room not found.");
                                ...
                                if (room != null) {
                                    ...
                                    if (roomContents != null && roomContents.size() != 0) {
                                        ...
                                        for (Integer clientId : clientIds) {
                                            if (originId.intValue() != clientId.intValue()) {
                                                RoomPacket tmpRoomPacket = new RoomPacket(new Header(originId, clientId), new Body(null, new RoomContentWrapper(roomContents)), roomId, RoomOp.ROOM_UPDATE);
                                                updateClientPacketMap(clientPacketMap, clientId, tmpRoomPacket);
                                            }
                                        }
